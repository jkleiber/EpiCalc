	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		enter.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				float wh=w;
				works[1]="P= ("+w+"!)/("+p+"! * "+diff+"!)";
				String str1 = String.valueOf(wh), str2 = String.valueOf(p),str3 = String.valueOf(dif);
				for(float r=wh-1;r>0;r--){
					wh*=r;
					str1+=" * "+String.valueOf(r);
				}
				for(float iv=dif-1;iv>0;iv--){
					dif*=iv;
					str3+=" * "+String.valueOf(iv);
				}
				for(float i=w-1;i>diff;i--){
					w*=i;
					str1+=" * "+String.valueOf(i);
				}
				
				
				for(float iii=p-1;iii>0;iii--){
					p*=iii;
					str2+=" * "+String.valueOf(iii);
				}
				...
				works[0]="Combinations= (whole group!)/(partial! * (whole-partial)!)";
				works[2]="P= ("+str1+")/("+str2+" * "+str3+")";
				works[3]="P= "+wh+"/"+dif+"= "+pp;
			}else{
				String err="NO INPUT";
